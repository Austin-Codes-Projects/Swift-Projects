...
let score = SKLabelNode(fontNamed: "0")
...
func setup() {
    ...
    //Set up the text, position, font size, and font color of the global SK Label Node intiated on Line 22, this is vital because without these paremeters set it can be hard to see the score on the application
    score.text = "0"
    score.position = CGPoint(x: 300, y: scene.height - 75)
    score.fontSize = 65
    score.fontColor = SKColor.black
    
    //Add the Score SK Label node to the scene to be shown to the user
...
func ballExitedScene() {
    ...
    scoreboard = hitTargets
    score.text = String(scoreboard)
